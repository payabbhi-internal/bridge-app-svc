...
//TODO refactor
func GetStringInterfaceParam(params map[string]interface{}, key string) (string, error) {
	if value, ok := params[key]; ok {
...
func GetRecordsParam(params map[string]interface{}, recordKey string, optional bool) ([]*Record, error) {
	var recordRequests []*Record
	if recordItems, ok := params[recordKey]; ok {
		...
		case []*Record:
			...
			for _, record := range vv {
				recordRequests = append(recordRequests, &Record{
